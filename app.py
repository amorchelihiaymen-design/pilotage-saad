...
def hhmm_to_decimal(hhmm_str):
    """Convertit '33:24' en 33.4 de manière robuste"""
    try:
        s = str(hhmm_str).strip()
        if not s or s == '0' or s == '0.0': return 0.0
        if ':' in s:
            h, m = map(int, s.split(':'))
            return h + (m / 60)
...
def robust_read_csv(file):
    """Lecture qui respecte les formats HH:MM et Décimaux"""
    try:
        df = pd.read_csv(file, sep=';', encoding='latin-1')
        for col in df.columns:
            # On ne touche pas aux colonnes qui contiennent des ":" (temps HH:MM)
            # On ne traite que les colonnes de chiffres avec virgule
            if df[col].dtype == 'object':
                sample = df[col].dropna().astype(str)
                if sample.str.contains(':').any():
                    df[col] = df[col].astype(str).str.strip() # On garde le texte HH:MM
                elif sample.str.contains(',').any() or sample.str.match(r'^-?\d+(\.\d+)?$').any():
...
if st.session_state.df_m is None:
    st.info("👋 Bonjour Aymen. Veuillez charger l'export Mensuel pour démarrer l'audit.")
else:
    # MAPPING SECTEURS
    ...
    if st.session_state.df_h is not None:
        ...
        def check_alert(row):
            ...
            t_contrat = row['Heures hebdo contrat']
    ...
    with tab_h:
        if st.session_state.df_h is not None:
            ...
            with a1:
                st.metric("Alertes 34h", len(df_filt_h[df_filt_h['Diagnostic'].str.contains("34h")]), delta="Risque Requalif.", delta_color="inverse")
            with a2:
                st.metric("Alertes 1/3 Contrat", len(df_filt_h[df_filt_h['Diagnostic'].str.contains("1/3")]), delta="Limite Légale", delta_color="inverse")
            with a3:
                st.metric("Alertes 40h (TP)", len(df_filt_h[df_filt_h['Diagnostic'].str.contains("40h")]), delta="Alerte Sécurité", delta_color="inverse")

            df_alerts = df_filt_h[df_filt_h['Diagnostic'] != "✅ Conforme"].copy()
            if not df_alerts.empty:
                st.error(f"Attention : {len(df_alerts)} anomalies détectées.")
